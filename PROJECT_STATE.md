...
| Release state | `release/current/` remains the obsolete SHA-bound artifact for `32275776039ebf39f40e7d8deef13417d2625f23`; this commit prepares the PHP-runner migration-004 hardening for a new release. No deployment or production migration has occurred. | artifact-observed plus current task boundary |
...
| Current phase | Migration-004 retry-idempotency is now implemented in the PHP migration runner with explicit schema validation; production remains on `f604148b836cbbc54956ce33b8eb08e92ccaebd3` with passwords unchanged and migration `004` inactive. | current source task plus directly verified prior production state |
| Next action | Generate the release from this pushed hardening commit when separately approved; do not deploy, apply migration `004`, or rotate credentials as part of release generation. | current task boundary |
