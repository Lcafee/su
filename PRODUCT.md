...
- `/menu` یک کنترل محلی برای نمایش دو‌ستونه یا فهرستی دارد و در نخستین بازدید
  نمایش دو‌ستونه را انتخاب می‌کند. `/menu2` فقط مسیر سازگاریِ انتقال به `/menu`
  است و در sitemap، QR یا ناوبری اصلی قرار نمی‌گیرد.
...
- تغییر منوی production از پنل مدیریت و publish انجام می‌شود، نه با build یا
  deploy کد.
- بخش عمومی `.htaccess` متعلق به release است؛ block حصاربندی‌شده‌ی runtime
  پارس‌پک state میزبان است و deploy باید آن را بدون تغییر حفظ کند.
...
- هویت Maps: Place ID برابر `ChIJHwiIQwA3vD8RouGWwXbq5GI`. مختصات تا زمان
  تأیید مستقل، `geo` معتبر محسوب نمی‌شود.
